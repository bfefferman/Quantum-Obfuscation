...
\item \textbf{An IND-CCA1-secure construction.} We give a construction for an IND-CCA1-secure symmetric-key encryption scheme for quantum states, under the assumption that quantum-secure one-way functions (qOWF) exist. These qOWFs are deterministic classical functions which are easy to compute, but hard to invert for quantum adversaries. For details, see~\cite{ABFGSS15}.
\end{enumerate}
