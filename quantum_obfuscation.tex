...
\wf{This next definition is pulled from Section 4 and its a bit out of place here, but I couldn't think of a better place to put it, now that Section 4 doesn't contain circuit definitions, do you have a better idea for this?}
Given two quantum circuit ensembles $\mathcal{C}=\{C_n\}_{n > 0}$ and $\mathcal{D}=\{D_n\}_{n>0}$, we say that $\mathcal{C}$ is functionally equivalent to $\mathcal{D}$ (denoted $\mathcal{C}\cong\mathcal{D}$) if the circuits themselves are functionally equivalent, i.e., if $\bigl\| C_n - D_n \bigr\|_\diamond \leq \negl(n)$ for all $n>0$.
...
\subsection{Definitions: Ensembles of states}
%%%%%%%%%%%%%%%%%%%%%%

%%The following has now been incorporated into Section 1.3, and so I commented this out, it's still here for reference
%\ga{It might be better to put this near the start, and use it for both VBB and IO sections.}
...
%
%A \textbf{circuit ensemble} is an infinite collection $\{C_s : s \in S\}$ of circuits indexed by some set $S \subset \{0, 1\}^*$, such that both the size and the number of qubits of $C_s$ are bounded by some fixed polynomial function of $|s|$. We will denote ensembles by italicized capital letters (optionally, with the indexing set in the subscript.) Elements of the ensemble will be denoted by the corresponding non-italicized capital letter (optionally, with the index in the subscript.) So, for example, we may write $\mathcal C_S := \{C_s : s \in S\}$. Just as in the single-circuit case, we will overload this notation for circuit ensembles and use it to also refer to the corresponding family of CPTP maps. A \textbf{uniform circuit ensemble} is a circuit ensemble together with a deterministic polynomial-time Turing Machine which, on input $s \in S$, outputs a classical description of $C_s$. Given two circuit ensembles $\mathcal C_S$ and $\mathcal D_S$, we say that $\mathcal C_S$ is functionally equivalent to $\mathcal D_S$ (denoted $\mathcal C_S \cong \mathcal D_S$) if the circuits themselves are functionally equivalent, i.e., if $\|C_s - D_s\|_\diamond \leq \negl(|s|)$ for all $s \in S$. 
\wf{There's a notation change in this section whereby circuit and state ensembles are indexed by elements of a set, instead of natural numbers, does this bother you?  I think it's ok.}
...
\ga{Should this definition somehow vary over the interpreters $\algo J$ as well? }
...
For the other direction, let $(\algo O, \algo J)$ be an indistinguishability obfuscator. Given a learner $\algo A$, define a simulator $\algo S$ by setting $\algo S = \algo A \circ \algo O$. Let $(\mathcal C, \mathcal R)$ be a (circuit, ensemble) pair as in the best-possible definition. By the indistinguishability property, $\algo O(\mathcal C) \approx \algo O (\mathcal R)$, from which it follows that 
$$
\algo A( \algo O(\mathcal C)) \approx \algo A( \algo O(\mathcal R)) = \algo S(\mathcal R)\,,
$$
as desired.
\end{proof}
